...
def _ensure_user_writable_dir(path: Path) -> None:
    """
    Ensure a directory exists and is writable by the current user.

    This repo often comes from Windows as read-only, which can lead to Linux
    permissions like 0555 inside Docker and break uploads/agent write tools.
    """
    ...
    try:
        ...
        if (mode & 0o200) == 0:
            os.chmod(path, mode | 0o200)  # add u+w, keep existing bits
    except Exception as exc:
        logger.warning("Unable to chmod directory %s: %s", path, exc)
