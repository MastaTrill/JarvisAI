...
class APIKeyManager:
    ...
    def validate_key(self, api_key: str) -> Optional[Dict[str, Any]]:
        ...
        for key_id, key_data in self._keys.items():
            if key_data["is_active"] and self._hasher.verify_password(
...
class RateLimiter:
    ...
    def __init__(self):
        self._buckets: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: {"tokens": config.RATE_LIMIT_REQUESTS, "last_update": time.time()}
        )
    ...
    def _sanitize_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if any(
                ...
                for sensitive in config.AUDIT_LOG_SENSITIVE_FIELDS
            ):
...
class RequestSigner:
    ...
    def verify_request(
        ...
        return hmac.compare_digest(signature, expected[config.REQUEST_SIGNATURE_HEADER])
