...
def _current_profile() -> str:
    stored = _get_task_memory().get_setting("agent_profile")
    if stored in {"safe", "dev", "full"}:
        return str(stored)
    value = _default_profile()
    _get_task_memory().set_setting("agent_profile", value)
    return value
...
def _set_profile(value: str) -> str:
    ...
    _get_task_memory().set_setting("agent_profile", v)
    return v
...
def _llm_mode() -> str:
    mode = (_get_task_memory().get_setting("agent_llm_mode") or "").strip().lower()
    if mode in {"fast", "quality"}:
        return mode
    mode = _llm_mode_default()
    _get_task_memory().set_setting("agent_llm_mode", mode)
    return mode
...
def _set_llm_mode(mode: str) -> str:
    ...
    _get_task_memory().set_setting("agent_llm_mode", m)
    return m


def _llm_model_override() -> str:
    return (_get_task_memory().get_setting("agent_llm_model") or "").strip()


def _set_llm_model_override(model_name: Optional[str]) -> str:
    model = str(model_name or "").strip()
    _get_task_memory().set_setting("agent_llm_model", model)
    return model
...
def _set_vision_provider(provider: str) -> str:
    ...
    _get_task_memory().set_setting("agent_vision_provider", p)
    return p


def _vision_model_override() -> str:
    return (_get_task_memory().get_setting("agent_vision_model") or "").strip()


def _set_vision_model_override(model_name: Optional[str]) -> str:
    model = str(model_name or "").strip()
    _get_task_memory().set_setting("agent_vision_model", model)
    return model
...
def _get_agent_policy() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("agent_policy")
...
def _set_agent_policy(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(data, dict):
        cfg.update(data)
    cfg["strict_confirm"] = bool(cfg.get("strict_confirm", True))
    _get_task_memory().set_setting("agent_policy", json.dumps(cfg))
    return cfg
...
def _get_control_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("agent_control_config")
...
def _set_control_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cfg["host_control_available"] = _is_truthy(
        os.getenv("JARVIS_HOST_CONTROL", ""))
    _get_task_memory().set_setting("agent_control_config", json.dumps(cfg))
    return cfg
...
def _resolve_workspace_context(
    ...
    resolved_workspace_id = (
        int(workspace_id) if workspace_id else _get_task_memory().get_active_workspace_id()
    )
    workspace = (
        _get_task_memory().get_project_workspace(resolved_workspace_id)
        if resolved_workspace_id
        else None
    )
    policy = _get_task_memory().get_workspace_policy(resolved_workspace_id)
...
def _dispatch_due_reminders(
    ...
    due_for_discord = (
        _get_task_memory().list_due_proactive_reminders(
            limit=limit, for_discord=True)
        if include_discord
        else []
    )
    due_for_voice = (
        _get_task_memory().list_due_proactive_reminders(limit=limit, for_voice=True)
        if include_voice
        else []
    )
    ...
    if (
        ...
        for reminder in due_for_discord:
            ...
            _get_task_memory().update_proactive_reminder(
...
def _get_local_voice_config() -> Dict[str, Any]:
    ...
    raw = _get_task_memory().get_setting("local_voice_config")
    if raw:
        try:
            parsed = json.loads(raw)
            if isinstance(parsed, dict):
...
def _set_local_voice_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _get_task_memory().set_setting("local_voice_config", json.dumps(cfg))
...
def _get_integration_config() -> Dict[str, Any]:
    ...
    raw = _get_task_memory().get_json_setting("elite_integration_config", {})
...
def _set_integration_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cfg["email_enabled"] = bool(cfg.get("email_enabled", False))
    cfg["email_to"] = str(cfg.get("email_to") or "").strip()
    _get_task_memory().set_json_setting(
...
def _desktop_presence_payload(*, workspace_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    if isinstance(workspace_id, int):
        active_workspace = _get_task_memory().get_project_workspace(workspace_id)
    if active_workspace is None:
        active_workspace_id = _get_task_memory().get_active_workspace_id()
        if active_workspace_id:
            active_workspace = _get_task_memory().get_project_workspace(
                active_workspace_id)
    snapshot = _get_task_memory().latest_desktop_presence_snapshot(
        workspace_id=workspace_id if isinstance(workspace_id, int) else None
    )
    reminders = _get_task_memory().list_proactive_reminders(
        limit=5,
        status="open",
        workspace_id=workspace_id if isinstance(workspace_id, int) else None,
    )
    next_actions = _get_task_memory().next_best_actions(
        workspace_id=workspace_id if isinstance(workspace_id, int) else None, limit=3
    )
    recent_vision = _get_task_memory().list_vision_observations(limit=1)
    return {
...
def _run_and_store_mission(
    *,
    ...
    if mission_id is not None:
        existing = _get_task_memory().get_mission_run(int(mission_id))
    ...
    saved = _get_task_memory().save_mission_run(
...
def _calendar_events_get() -> List[Dict[str, Any]]:
    items = _get_task_memory().get_json_setting("elite_calendar_events", [])
...
def _calendar_events_set(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items[:200]:
        if isinstance(item, dict):
            clean.append(item)
    _get_task_memory().set_json_setting("elite_calendar_events", clean)
    return clean
...
def _watcher_network_payload() -> Dict[str, Any]:
    jobs = _get_task_memory().list_autonomous_jobs(limit=200, mode="watcher")
    cfg = _get_integration_config()
    latest_presence = _get_task_memory().latest_desktop_presence_snapshot()
...
def _trust_receipts_payload(
    *, limit: int = 20, session_id: Optional[str] = None
    ...
    tool_runs = _get_task_memory().list_tool_executions(
        limit=limit, session_id=session_id)
    missions = _get_task_memory().list_mission_runs(limit=max(5, min(limit, 20)))
    receipts = []
    rollback_receipts = []
...
def _get_wakeword_config() -> Dict[str, Any]:
    ...
    raw = _get_task_memory().get_setting("voice_wakeword_config")
    if raw:
        try:
            parsed = json.loads(raw)
            if isinstance(parsed, dict):
...
def _set_wakeword_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cfg["threshold"] = max(
        0.05, min(float(cfg.get("threshold", 0.45) or 0.45), 0.99))
    cfg["chunk_ms"] = max(160, min(int(cfg.get("chunk_ms", 960) or 960), 4000))
    _get_task_memory().set_setting("voice_wakeword_config", json.dumps(cfg))
...
def _get_browser_workflow_templates() -> List[Dict[str, Any]]:
    raw = _get_task_memory().get_setting("browser_workflow_templates")
...
def _get_pending_approval(session_id: str) -> Optional[Dict[str, Any]]:
    raw = _get_task_memory().get_setting(_pending_approval_key(session_id))
    if not raw:
        return None
    try:
        parsed = json.loads(raw)
...
def _clear_pending_approval(session_id: str) -> bool:
    return _get_task_memory().delete_setting(_pending_approval_key(session_id))
...
def _run_autonomous_mission(
    *,
    ...
    plan = _get_task_memory().next_best_actions(
...
def _record_verified_tool_run(
    *,
    ...
    log_id = _get_task_memory().log_tool_execution(
        tool_name=tool_name,
...
def _tool_quantum_superposition(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _get_task_memory().add_quantum_event(
...
def _tool_quantum_decipher(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if event_type == "":
        event_type = None
    events = _get_task_memory().list_quantum_events(
...
def _get_quantum_alert_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("quantum_alert_config")
...
def _set_quantum_alert_config(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _get_task_memory().set_setting("quantum_alert_config", json.dumps(safe_cfg))
    return safe_cfg
...
def _evaluate_quantum_alerts(cfg: Dict[str, Any]) -> Dict[str, Any]:
    if not bool(cfg.get("enabled", True)):
        return {
            "active": False,
            "alerts": [],
            "stats": _get_task_memory().quantum_stats(hours=int(cfg.get("window_hours", 24))),
        }
...
def _tool_browser_workflow(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    stored_session = (
        _get_task_memory().get_browser_session(
...
def _memory_context_for_prompt(message: str, limit: int = 4) -> str:
    bundle = _get_task_memory().memory_context_bundle(
        query=message, limit=max(4, limit))
    matches = bundle.get("items") or []
    active_workspace_id = _get_task_memory().get_active_workspace_id()
    workspace = (
        _get_task_memory().get_project_workspace(active_workspace_id)
        if active_workspace_id
        else None
    )
...
def _run_project_watcher(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    workspace_id = (
        metadata.get("workspace_id") or _get_task_memory().get_active_workspace_id()
    )
    ...
    if watcher_type == "project":
        ...
        if not triggers:
            generated = _get_task_memory().generate_proactive_reminders(
    ...
    if watcher_type == "email":
        ...
        briefing = _get_task_memory().memory_briefing(
...
def _execute_autonomous_job(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if mode == "briefing":
        ...
        reminders = _get_task_memory().generate_proactive_reminders(
            workspace_id=_get_task_memory().get_active_workspace_id(),
            limit=3,
        )
        delivery = _dispatch_briefing_deliveries(briefing)
        return {
...
def activate_advanced_features():
    ...
    existing = _get_task_memory().list_goal_schedules(limit=500)
    ...
    for d in defaults:
        ...
        if g in existing_goals:
            continue
        sid = _get_task_memory().create_goal_schedule(
            goal=str(d["goal"]),
...
def list_tasks(
    ...
    tasks = _get_task_memory().list_tasks(
...
def create_task(payload: AgentTaskCreateRequest):
    task_id = _get_task_memory().create_task(
...
def update_task_status(task_id: int, payload: AgentTaskStatusRequest):
    ok = _get_task_memory().update_task_status(
...
def quantum_stats(hours: int = Query(default=24, ge=1, le=24 * 365)):
    return _get_task_memory().quantum_stats(hours=hours)
...
def _get_quantum_remediation_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("quantum_remediation_config")
...
def _set_quantum_remediation_config(data: Dict[str, Any]) -> Dict[str, Any]:
    cfg = _quantum_remediation_default()
    cfg.update(data)
    _get_task_memory().set_setting("quantum_remediation_config", json.dumps(cfg))
    return cfg
...
def _get_quantum_notification_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("quantum_notification_config")
...
def _set_quantum_notification_config(data: Dict[str, Any]) -> Dict[str, Any]:
    cfg = _quantum_notification_default()
    cfg.update(data)
    _get_task_memory().set_setting("quantum_notification_config", json.dumps(cfg))
    return cfg
...
def _get_quantum_rbac_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("quantum_rbac_config")
...
def _set_quantum_rbac_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cfg["role"] = role
    _get_task_memory().set_setting("quantum_rbac_config", json.dumps(cfg))
    return cfg
...
def _audit_quantum_op(op_type: str, status: str, details: Dict[str, Any]) -> None:
    try:
        _get_task_memory().add_quantum_ops_audit(
...
def _memory_saved_filters_get() -> List[Dict[str, Any]]:
    raw = _get_task_memory().get_setting("memory_archived_saved_filters")
    if not raw:
        return []
    try:
        parsed = json.loads(raw)
...
def _get_briefing_delivery_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("memory_briefing_delivery_config")
...
def _get_quantum_incidents() -> List[Dict[str, Any]]:
    raw = _get_task_memory().get_setting("quantum_incidents")
    if not raw:
        return []
    try:
        parsed = json.loads(raw)
...
def _set_quantum_incidents(items: List[Dict[str, Any]]) -> None:
    _get_task_memory().set_setting("quantum_incidents", json.dumps(items))
...
def _quantum_alert_correlations(*, hours: int, window_minutes: int) -> Dict[str, Any]:
    ...
    events = _get_task_memory().list_quantum_events(limit=5000, since_hours=hours)
...
def _compute_quantum_baselines(hours: int) -> Dict[str, Any]:
    ...
    events = _get_task_memory().list_quantum_events(limit=5000, since_hours=h)
...
def _get_quantum_baselines() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("quantum_baselines")
    if not raw:
        return {}
    try:
        parsed = json.loads(raw)
...
def _set_quantum_baselines(baseline: Dict[str, Any]) -> Dict[str, Any]:
    _get_task_memory().set_setting("quantum_baselines", json.dumps(baseline))
    return baseline
...
def _quantum_root_cause_graph(
    *, incident_id: Optional[str], hours: int
    ...
    events = _get_task_memory().list_quantum_events(limit=1000, since_hours=hours)
...
def _quantum_risk_score(*, horizon_hours: int) -> Dict[str, Any]:
    ...
    stats = _get_task_memory().quantum_stats(hours=24)
    alerts_eval = _evaluate_quantum_alerts(_get_quantum_alert_config())
    anomalies = _quantum_anomalies(
        _get_task_memory().list_quantum_events(limit=5000, since_hours=24),
        hours=24,
        z_threshold=2.0,
    )
...
def _get_quantum_playbook_v2_config() -> Dict[str, Any]:
    raw = _get_task_memory().get_setting("quantum_playbook_v2_config")
...
def _set_quantum_playbook_v2_config(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    _get_task_memory().set_setting("quantum_playbook_v2_config", json.dumps(cfg))
    return cfg
...
def _run_quantum_experiment(
    *, preset: str, measure_count: Optional[int], entangle_count: Optional[int]
    ...
    events = _get_task_memory().list_quantum_events(limit=5000, since_hours=24)
    decipher = _quantum_decipher_analysis(events, hours=24)
    snap_id = _get_task_memory().create_quantum_decipher_snapshot(decipher)
    result = {
        "preset": preset,
...
def _run_quantum_remediation(*, hours: int, force: bool) -> Dict[str, Any]:
    ...
    post_events = _get_task_memory().list_quantum_events(
...
def quantum_decipher_snapshots(limit: int = Query(default=20, ge=1, le=200)):
    return {"snapshots": _get_task_memory().list_quantum_decipher_snapshots(limit=limit)}


@router.get("/quantum/ops-audit")
def quantum_ops_audit(limit: int = Query(default=100, ge=1, le=500)):
    return {"items": _get_task_memory().list_quantum_ops_audit(limit=limit)}
...
def quantum_noc(hours: int = Query(default=24, ge=1, le=24 * 365)):
    ...
    snaps = _get_task_memory().list_quantum_decipher_snapshots(limit=2)
    delta = None
    if len(snaps) >= 2:
...
def quantum_incident_workspace(incident_id: Optional[str] = None):
    ...
    if target:
        incident_annotations = _get_task_memory().list_quantum_annotations(
    ...
    async def event_gen():
        for _ in range(300):
            ...
            payload = {
                ...
                "recent_audit": _get_task_memory().list_quantum_ops_audit(limit=10),
...
def quantum_annotations_create(payload: QuantumAnnotationCreateRequest):
    aid = _get_task_memory().add_quantum_annotation(
...
def quantum_annotations_list(
    ...
    return {
        "annotations": _get_task_memory().list_quantum_annotations(
...
def quantum_memory_graph(hours: int = Query(default=24, ge=1, le=24 * 365)):
    events = _get_task_memory().list_quantum_events(limit=400, since_hours=hours)
    alerts_eval = _evaluate_quantum_alerts(_get_quantum_alert_config())
    audits = _get_task_memory().list_quantum_ops_audit(limit=100)
    nodes: List[Dict[str, Any]] = []
    edges: List[Dict[str, Any]] = []
...
def quantum_agent_replay(
    ...
    runs = _get_task_memory().list_goal_runs(limit=limit, session_id=session_id)
...
def quantum_replay(
    ...
    events = _get_task_memory().list_quantum_events(limit=5000, since_hours=24 * 365)
...
def quantum_sandbox_run(payload: QuantumSandboxRunRequest):
    ...
    fake_health = _quantum_health_score(
        stats=_get_task_memory().quantum_stats(hours=payload.hours),
        alerts=merged_alerts,
        anomalies=_quantum_anomalies(
            _get_task_memory().list_quantum_events(
...
def _execute_goal_run(
    *, goal: str, session_id: Optional[str], auto_approve: bool
    ...
    task_id = _get_task_memory().create_task(goal_text, session_id=sid)
    _get_task_memory().update_task_status(
        task_id, "in_progress", note="Goal runner started")
    run_id = _get_task_memory().create_goal_run(
    ...
    _get_task_memory().update_goal_run(
...
def create_goal_schedule(payload: GoalScheduleCreateRequest):
    schedule_id = _get_task_memory().create_goal_schedule(
...
def remember_memory(payload: LongTermMemoryCreateRequest):
    memory_id = _get_task_memory().create_long_term_memory(
...
def search_memories(
    ...
    return {"items": _get_task_memory().search_long_term_memories(query=query, limit=limit)}
...
def list_long_term_memories(
    ...
    return {
        "items": _get_task_memory().list_long_term_memories(
            limit=limit,
...
def memory_overview(
    ...
    return _get_task_memory().memory_overview(
...
def memory_profile(
    ...
    overview = _get_task_memory().memory_overview(
...
def list_project_workspaces(
    ...
    items = _get_task_memory().list_project_workspaces(
...
def create_project_workspace(payload: WorkspaceCreateRequest):
    try:
        workspace = _get_task_memory().create_project_workspace(
            name=payload.name,
...
def set_active_project_workspace(payload: WorkspaceActivationRequest):
    try:
        result = _get_task_memory().set_active_workspace(payload.workspace_id)
    except ValueError as exc:
...
def get_current_workspace_policy():
    ...
    return {
        "workspace": workspace,
        "policy": _get_task_memory().get_workspace_policy(active_workspace_id),
    }


@router.get("/memory/workspaces/{workspace_id}/policy")
def get_workspace_policy_route(workspace_id: int):
    workspace = _get_task_memory().get_project_workspace(workspace_id)
    if workspace is None:
        raise HTTPException(status_code=404, detail="workspace not found")
    return {
        "workspace": workspace,
        "policy": _get_task_memory().get_workspace_policy(workspace_id),
    }
...
def set_workspace_policy_route(workspace_id: int, payload: WorkspacePolicyRequest):
    ...
    if workspace is None:
        raise HTTPException(status_code=404, detail="workspace not found")
    policy = _get_task_memory().set_workspace_policy(
        workspace_id,
...
def update_project_workspace(workspace_id: int, payload: WorkspaceUpdateRequest):
    ok = _get_task_memory().update_project_workspace(
        workspace_id,
        name=payload.name,
        description=payload.description,
        focus=payload.focus,
    ...
    if not ok:
        raise HTTPException(status_code=404, detail="workspace not found")
    return {"ok": True, "workspace": _get_task_memory().get_project_workspace(workspace_id)}
...
def memory_graph(
    ...
    return _get_task_memory().workspace_memory_graph(workspace_id=workspace_id, limit=limit)
...
def proactive_reminders(
    ...
    return {
        "items": _get_task_memory().list_proactive_reminders(
            limit=limit,
            status=status,
            workspace_id=workspace_id,
...
def create_proactive_reminder(payload: ReminderCreateRequest):
    try:
        reminder = _get_task_memory().create_proactive_reminder(
            title=payload.title,
...
def generate_proactive_reminders(
    ...
    return _get_task_memory().generate_proactive_reminders(
        workspace_id=workspace_id, limit=limit
    )
...
def trust_report(
    ...
    if session_id:
        recent = _get_task_memory().list_tool_executions(
            limit=limit, session_id=session_id)
        return {
            "session_id": session_id,
    ...
    return _get_task_memory().tool_reliability_report(limit=limit)
...
def next_action(
    ...
    return _get_task_memory().next_best_actions(workspace_id=workspace_id, limit=limit)
...
def execute_next_action(payload: NextActionExecuteRequest):
    ...
    if kind == "activate_workspace":
        ...
        result = _get_task_memory().set_active_workspace(workspace_id)
    ...
    if kind == "reminder_done":
        ...
        ok = _get_task_memory().update_proactive_reminder(
            reminder_id, status="done", delivered=True
        )
        reminder = _get_task_memory().get_proactive_reminder(reminder_id)
        return {"ok": ok, "kind": kind, "session_id": session_id, "result": reminder}
    if kind == "workspace_policy":
        workspace_id = execution.get("workspace_id")
        policy = _get_task_memory().get_workspace_policy(
            workspace_id if isinstance(workspace_id, int) else None
        )
        workspace = (
            _get_task_memory().get_project_workspace(workspace_id)
            if isinstance(workspace_id, int)
            else None
        )
        return {
...
def autonomy_mission_list(
    ...
    return {
        "items": _get_task_memory().list_mission_runs(limit=limit, workspace_id=workspace_id)
    }


@router.get("/autonomy/missions/{mission_id}")
def autonomy_mission_get(mission_id: int):
    item = _get_task_memory().get_mission_run(mission_id)
    if item is None:
...
def desktop_presence_set(payload: DesktopPresenceSnapshotRequest):
    ...
    snapshot = _get_task_memory().save_desktop_presence_snapshot(
        workspace_id=workspace_id,
...
def browser_sessions(
    ...
    return {
        "items": _get_task_memory().list_browser_sessions(
...
def browser_sessions_save(payload: BrowserSessionCreateRequest):
    try:
        item = _get_task_memory().save_browser_session(
            name=payload.name,
...
def browser_sessions_delete(
    ...
    if not _get_task_memory().delete_browser_session(target):
        raise HTTPException(
            status_code=404, detail="browser session not found")
    return {"ok": True, "items": _get_task_memory().list_browser_sessions(limit=50)}
...
def browser_sessions_health(payload: BrowserSessionHealthCheckRequest):
    ...
    if payload.session_id or payload.session_name:
        ...
        session = _get_task_memory().get_browser_session(target)
        if session is None:
            raise HTTPException(
                status_code=404, detail="browser session not found")
        sessions = [session]
    else:
        sessions = _get_task_memory().list_browser_sessions(
    ...
    for session in sessions:
        try:
            ...
            updated = _get_task_memory().update_browser_session_health(
...
def update_proactive_reminder(reminder_id: int, payload: ReminderUpdateRequest):
    ...
    return {"ok": True, "reminder": _get_task_memory().get_proactive_reminder(reminder_id)}
...
def memory_briefing(
    ...
    return _get_task_memory().memory_briefing(
        period=period, recent_project_hours=recent_project_hours
    )
...
def consolidate_long_term_memories(limit: int = Query(default=200, ge=10, le=1000)):
    return _get_task_memory().consolidate_long_term_memories(limit=limit)


@router.post("/memory/{memory_id}")
def update_long_term_memory(memory_id: int, payload: LongTermMemoryUpdateRequest):
    ok = _get_task_memory().update_long_term_memory(
        memory_id,
        content=payload.content,
        tags=payload.tags,
        importance=payload.importance,
...
def delete_long_term_memory(memory_id: int):
    ok = _get_task_memory().archive_long_term_memory(memory_id, archived=True)
    if not ok:
        raise HTTPException(status_code=404, detail="memory not found")
...
def pin_long_term_memory(memory_id: int, pinned: bool = Query(default=True)):
    ok = _get_task_memory().update_long_term_memory(memory_id, pinned=pinned)
    if not ok:
        raise HTTPException(status_code=404, detail="memory not found")
...
def bulk_memory_action(payload: LongTermMemoryBulkActionRequest):
    return _get_task_memory().bulk_update_long_term_memories(
...
def reorder_pinned_memories(payload: LongTermMemoryPinOrderRequest):
    return _get_task_memory().reorder_pinned_memories(payload.ids)


@router.post("/memory/projects/reorder")
def reorder_project_memories(payload: LongTermMemoryProjectOrderRequest):
    return _get_task_memory().reorder_project_memories(payload.ids)
...
def list_memory_briefing_automations():
    return {"items": _get_task_memory().list_autonomous_jobs(limit=50, mode="briefing")}


@router.post("/memory/briefings/automations")
def ensure_memory_briefing_automations(payload: MemoryBriefingAutomationRequest):
    items = _get_task_memory().ensure_daily_briefing_automations(
        session_id=payload.session_id,
...
def memory_briefing_delivery_test(
    ...
    briefing = _get_task_memory().memory_briefing(period=period)
    delivery = _dispatch_briefing_deliveries(briefing)
...
def control_browser_workflow(payload: BrowserWorkflowRequest):
    ...
    if payload.session_name:
        loaded_session = _get_task_memory().get_browser_session(payload.session_name)
    ...
    if payload.save_session and payload.session_name:
        ...
        saved_session = _get_task_memory().save_browser_session(
    ...
    if payload.session_name:
        _get_task_memory().touch_browser_session(payload.session_name)
    verification = _record_verified_tool_run(
...
async def vision_analyze(
    ...
    obs_id = _get_task_memory().add_vision_observation(
...
def list_vision_observations(limit: int = Query(default=20, ge=1, le=100)):
    return {"items": _get_task_memory().list_vision_observations(limit=limit)}


@router.post("/autonomy/jobs")
def create_autonomous_job(payload: AutonomousJobCreateRequest):
    job_id = _get_task_memory().create_autonomous_job(
        name=payload.name,
        goal=payload.goal,
        mode=payload.mode,
        interval_minutes=payload.interval_minutes,
...
def list_autonomy_jobs(
    ...
    return {"items": _get_task_memory().list_autonomous_jobs(limit=limit, mode=mode)}


@router.get("/autonomy/watchers")
def list_project_watchers(limit: int = Query(default=50, ge=1, le=200)):
    return {"items": _get_task_memory().list_autonomous_jobs(limit=limit, mode="watcher")}
...
def ensure_project_watcher(payload: ProjectWatcherRequest):
    ...
    workspace = (
        _get_task_memory().get_project_workspace(payload.workspace_id)
    ...
    existing = None
    for job in _get_task_memory().list_autonomous_jobs(limit=200, mode="watcher"):
    ...
    if existing:
        _get_task_memory().update_autonomous_job(
            int(existing["id"]),
            name=name,
            goal=goal,
            enabled=payload.enabled,
    ...
    job = next(
        (
            item
            for item in _get_task_memory().list_autonomous_jobs(limit=200, mode="watcher")
...
def update_autonomy_job(job_id: int, payload: AutonomousJobUpdateRequest):
    ok = _get_task_memory().update_autonomous_job(
        job_id,
        enabled=payload.enabled,
        interval_minutes=payload.interval_minutes,
        auto_approve=payload.auto_approve,
...
def run_autonomy_job(job_id: int):
    jobs = _get_task_memory().list_autonomous_jobs(limit=500)
    target = next((j for j in jobs if int(j["id"]) == int(job_id)), None)
    if target is None:
        raise HTTPException(status_code=404, detail="autonomous job not found")
    result = _execute_autonomous_job(target)
    _get_task_memory().mark_autonomous_job_result(
...
def agent_chat(payload: AgentChatRequest):
    ...
    if "who am i" in msg_lower:
        overview = _get_task_memory().memory_overview(limit_per_group=6)
    ...
    if workspace_switch:
        ...
        workspaces = _get_task_memory().list_project_workspaces(
        ...
        if target is None:
            ...
            result = _get_task_memory().set_active_workspace(int(target["id"]))
    ...
    if "what do you remember" in msg_lower or "what do you know about me" in msg_lower:
        overview = _get_task_memory().memory_overview(limit_per_group=6)
        matches = _get_task_memory().list_long_term_memories(limit=8)
        if not matches:
