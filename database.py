...
def create_default_users():
    ...
    with get_db_session() as db:
        for user_data in default_users:
            ...
            existing_user = (
                db.query(User).filter_by(username=user_data["username"]).first()
            )
            if existing_user:
...
class DatabaseManager:
    ...
    def cleanup_old_data(days: int = 30):
        ...
        with get_db_session() as db:
            ...
            deleted_quantum = (
                db.query(QuantumState)
                .filter(QuantumState.created_at < cutoff_date)
                .delete()
            )

            # Clean up old temporal patterns
            deleted_temporal = (
                db.query(TemporalPattern)
                .filter(TemporalPattern.created_at < cutoff_date)
                .delete()
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "⚠️ Install passlib[bcrypt] to create default users: pip install passlib[bcrypt]"
        )
